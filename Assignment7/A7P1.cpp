...
int main() {
  ...
  double tau; cin >> tau;
  std::vector< std::pair<double,double> > bifur;
  ...
  xv[1] = theta_old;
  xv[2] = omega;
  //xv[3] = gamma; It's a wrong trial.
  double accel = pendulumAcc( xv );
  theta_old = theta_old - omega*tau + 0.5*tau*tau*accel;    
  ...
  std::vector<double> ga_plot;
  std::vector<double> period;
  std::vector< std::pair<double,double> > poincare_map;
  double dt_min = tau;
  double dt_max = tau;
  int iStep;
  for( iStep=count*nStep; iStep<(count+1)*nStep; iStep++ ) {  
    ...
    if ( iperiod != iperiod_old ) {
      ...
      bifur.push_back( std::pair<double,double>(gamma, thplot) );
      //std::cout << "Period reached. theta = " <<  gaplot << ", omega = " << thplot*180/pi << endl;
    }
  ...
  ofstream bifurOut("bifur_plot.txt");
  for( int  i=0; i< bifur.size(); i++ ) {
  bifurOut << bifur[i].first << " "  << bifur[i].second << endl;
  }
  bifurOut.close();
